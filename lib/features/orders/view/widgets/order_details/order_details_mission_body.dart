...
class _OrderDetailsMissionBodyState extends State<OrderDetailsMissionBody> {
    ...
    if (_isEventAssistance) {
      ...
      if (task != null && task.isNotEmpty) {
        ...
        return <_TaskItem>[
          _TaskItem(
            label: task,
            detail: hours == null ? null : EventAssistanceOrderHelper.formatHoursDetail(hours),
          ),
        ];
      }
  ...
  bool get _isWaitingCustomer =>
      OrderLifecyclePolicy.isAwaitingCustomerCompletion(_effectiveStatus(widget.bloc.state));

  bool get _isExtensionRequested =>
      _effectiveStatus(widget.bloc.state) == CleaningBookingStatus.timeExtensionRequested;
  ...
  Future<void> _showCompletionMessageSheet() async {
                ...
                children: [
                  ...
                  Expanded(
                    child: FilledButton(
                      ...
                      style: FilledButton.styleFrom(backgroundColor: const Color(0xff1DBCC8), foregroundColor: Colors.white),
    ...
    widget.bloc.add(CompleteOrderUsecaseEvent(
      params: CompleteOrderUsecaseParams(id: widget.order.id!, completionMessage: trimmed),
    ));
  }
                    ...
                    child: OutlinedButton(
                      onPressed: widget.order.id == null
                          ...
                          : () {
                              ...
                              widget.bloc.add(FetchOrderDetailsUsecaseEvent(params: FetchOrderDetailsUsecaseParams(id: widget.order.id!)));
                            },
  ...
  Widget build(BuildContext context) {
    return BlocListener<OrdersBloc, OrdersState>(
      ...
      listenWhen: (previous, current) =>
          (previous.completeOrderUsecaseStatus != current.completeOrderUsecaseStatus && current.completeOrderUsecaseStatus == BlocStatus.success) ||
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(child: AppText.headlineMedium('تفاصيل الطلب ${widget.order.bookingNumber ?? ''}', textAlign: TextAlign.center)),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              AppText.labelLarge(_serviceDate(), color: Colors.white),
              AppText.labelLarge(_missionStatusText, color: Colors.white, fontWeight: FontWeight.bold),
            ],
          ),
          10.verticalSpace,
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
            decoration: BoxDecoration(color: Colors.white.withAlpha(32), borderRadius: BorderRadius.circular(12)),
            child: Column(
              children: [
                ...
                if (_timerHelperText != null) ...[
                  6.verticalSpace,
                  AppText.bodySmall(_timerHelperText!, color: Colors.white, textAlign: TextAlign.center),
                ],
              ],
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            return Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: _isChecklistLocked ? const Color(0xffF1F5F9) : const Color(0xffF8FAFC),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: _isChecklistLocked ? const Color(0xffCBD5E1) : const Color(0xffE5E7EB)),
                ),
                child: Row(
                  children: [
                    Checkbox(
                      value: checked,
                      onChanged: _isChecklistLocked ? null : (value) => setState(() => _taskState[taskKey] = value ?? false),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          AppText.bodyMedium(task.label, fontWeight: FontWeight.w700, color: _isChecklistLocked ? const Color(0xff64748B) : null),
                          if (task.detail != null) AppText.bodySmall(task.detail!, color: const Color(0xff9CA3AF)),
                        ],
                      ),
                    ),
                  ],
          ...
          if (!_isChecklistLocked && !_allTasksChecked) ...[
            2.verticalSpace,
            AppText.bodySmall('يرجى تحديد جميع المهام قبل إنهاء العمل', color: context.error),
          ],
        ],
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          WorkerPaymentSummary(
            ...
            travelFee: widget.order.myAssignment?.travelFee ?? widget.order.travelFee,
            addonsTotal: widget.order.addonsTotal,
            totalPrice: widget.order.totalPrice,
            showAddonsTotal: false,
            useWorkerShare: widget.order.myAssignment != null,
            serviceShareAmount: widget.order.myAssignment?.serviceShareAmount,
        ...
        return Column(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: const Color(0xffEEF2FF), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xffCBD5E1))),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(
                    children: [
                      const Icon(Icons.hourglass_top_rounded, color: Color(0xff1E2A78)),
                      8.horizontalSpace,
                      Expanded(child: AppText.bodyMedium('تم إرسال طلب إنهاء العمل إلى العميل. بانتظار التأكيد أو طلب إجراء آخر.', color: const Color(0xff1E2A78), textAlign: TextAlign.start)),
                    ],
                  ),
                  if (completionMessage != null) ...[
                    10.verticalSpace,
                    AppText.labelMedium('رسالتك للعميل:', color: const Color(0xff1E2A78), fontWeight: FontWeight.w700),
                    4.verticalSpace,
                    AppText.bodySmall(completionMessage, color: const Color(0xff374151), textAlign: TextAlign.start),
                  ],
                ],
              ),
            ),
  ...
  Widget _buildFinishButton() {
    return BlocBuilder<OrdersBloc, OrdersState>(
      ...
      builder: (context, state) {
        ...
        return FilledButton(
          onPressed: !canSendFinishRequest ? null : () => unawaited(_showCompletionMessageSheet()),
          style: FilledButton.styleFrom(backgroundColor: const Color(0xff1DBCC8), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14)),
          child: loading
              ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
              : AppText.labelLarge(_finishButtonText, color: Colors.white, fontWeight: FontWeight.w700),
        );
      },
    );
  }
