...
import '../../features/main/navigation/main_tab_navigation.dart';
...
class CleaningWorkerGlobalPromptCoordinator {
  ...
  final Map<int, _ExtensionPromptState> _extensionPromptStates =
      <int, _ExtensionPromptState>{};
  ...
  bool _shouldHandleRealtimeEvent(
    String eventName,
    Map<String, dynamic> payload,
  ) {
  ...
  Future<bool> _openExtensionPromptForWarning({
    ...
    if (state == _ExtensionPromptState.showing ||
        state == _ExtensionPromptState.pendingPrompt ||
        state == _ExtensionPromptState.resolved) {
      return false;
    }
    ...
    try {
      ...
      _extensionPromptStates[warningId] = resolved
          ? _ExtensionPromptState.resolved
          : _ExtensionPromptState.idle;
      return resolved;
    } catch (_) {
